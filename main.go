...
func main() {
	...
	for msg := range iter {
		err = callMeasuringPoint(caller, msg)
		if err != nil {
			msg.Fail()
			l.Error(err)
			continue
...
func callMeasuringPoint(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	defer func() {
		if e := recover(); e != nil {
	...
	measuringPoint, equipment := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetMeasuringPoint(measuringPoint, equipment, accepter)
	return nil
}

func extractData(data map[string]interface{}) (measuringPoint, equipment string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	measuringPoint = sdc.MeasuringPoint.MeasuringPoint
	equipment = sdc.MeasuringPoint.Equipment
	return
}
